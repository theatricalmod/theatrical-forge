...
public class CableBlockEntity extends TileEntity implements IEnergyStorage, ITickableTileEntity {
    ...
    public CableBlockEntity() {
        super(TheatricalBlockEntities.CABLE);
    }

//    public CableType[] cableTypes = new CableType[]{CableType.NONE, CableType.NONE, CableType.NONE, CableType.NONE};
...
//        nbtTagCompound.putIntArray("types", Arrays.stream(cableTypes).mapToInt(CableType::getIndex).toArray());
...
//        this.cableTypes = Arrays.stream(nbt.getIntArray("types")).mapToObj(CableType::byIndex).toArray(CableType[]::new);
    ...
    public boolean isConnected(Direction direction, CableType cableType) {
        ...
        if(tileEntity instanceof CableBlockEntity){
            CableBlockEntity cableBlockEntity = (CableBlockEntity)tileEntity;
            return cableBlockEntity.hasType(cableType);
        }
        if (hasType(CableType.POWER)) {
...
//    }
    ...
    public void doEnergyTransfer() {
        ...
        for (Direction face : Direction.values()) {
            ...
            if (tile == null) {
                continue;
            } else if (tile instanceof CableBlockEntity) {
